...
export async function getPosts(maxNumber) {
  ...
  const stmt = db.prepare(`
    SELECT posts.id, imageUrl AS image, title, content, created_at AS createdAt, first_name AS userFirstName, last_name AS userLastName, COUNT(likes.post_id) AS likes, EXISTS(SELECT * FROM likes WHERE likes.post_id = posts.id and likes.user_id = 2) AS isLiked
    FROM posts
...
export async function storePost(post) {
  const stmt = db.prepare(`
    INSERT INTO posts (imageUrl, title, content, user_id)
    VALUES (?, ?, ?, ?)`);
  await new Promise((resolve) => setTimeout(resolve, 1000));
